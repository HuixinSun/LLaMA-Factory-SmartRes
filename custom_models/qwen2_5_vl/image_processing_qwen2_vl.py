    ...
    def _preprocess(
        ...
        for image in images:
            if do_resize:
                ...
                # resamples the image array to (resized_height, resized_width) 
                image = resize(
                    image, size=(resized_height, resized_width), resample=resample, input_data_format=input_data_format
                )

            if do_rescale:
                # Multiplies pixel values by rescale_factor (default 1/255) to map 0–255 → 0–1
                image = self.rescale(image, scale=rescale_factor, input_data_format=input_data_format)

            if do_normalize:
                # Per-channel standardization: (image − mean) / std
        ...
        grid_t = patches.shape[0] // temporal_patch_size # number of temporal groups = total frames / temporal_patch_size.
        grid_h, grid_w = resized_height // patch_size, resized_width // patch_size # spatial grid sizes: H/patch_size, W/patch_size.
        # split patches into temporal groups, then spatial groups, then merge spatial groups, then merge temporal groups.
        ...
        # final tokens shaped as (num_tokens, token_dim)
        # num_tokens = grid_t × grid_h × grid_w
        # token_dim = C × temporal_patch_size × patch_size × patch_size
