...
def _create_selector_lr_optimizer(
    model: "PreTrainedModel",
    training_args: "TrainingArguments",
    finetuning_args: "FinetuningArguments",
) -> "torch.optim.Optimizer":
    """Create optimizer with separate learning rate for MTS selector modules."""
    default_lr = training_args.learning_rate
    selector_lr = finetuning_args.selector_learning_rate

    decay_param_names = _get_decay_parameter_names(model)
    
    # Separate parameters into selector and non-selector groups
    param_groups = {
        "selector_decay": [],
        "selector_nodecay": [],
        "other_decay": [],
    ...
    selector_param_count = 0
    other_param_count = 0
    selector_param_names = []
    
    for name, param in model.named_parameters():
        if param.requires_grad:
            is_selector = _is_selector_param(name)
            has_decay = name in decay_param_names
            
            if is_selector:
                selector_param_count += param.numel()
                selector_param_names.append(name)
                if has_decay:
                    param_groups["selector_decay"].append(param)
                else:
                    param_groups["selector_nodecay"].append(param)
            else:
                other_param_count += param.numel()
                if has_decay:
                    param_groups["other_decay"].append(param)
                else:
                    param_groups["other_nodecay"].append(param)
    
    # Debug: print selector params found
    if selector_param_names:
        print(f"[DEBUG] Selector params found: {selector_param_names[:10]}{'...' if len(selector_param_names) > 10 else ''}")
    else:
        # Print some sample param names to debug
        sample_names = [n for n, p in model.named_parameters() if p.requires_grad][:20]
        print(f"[DEBUG] No selector params found. Sample trainable param names:\n" + "\n".join(sample_names))
    ...
    optimizer_grouped_parameters = [
        {
            "params": param_groups["selector_decay"],
            "lr": selector_lr,
            "weight_decay": training_args.weight_decay,
        },
        {
            "params": param_groups["selector_nodecay"],
            "lr": selector_lr,
            "weight_decay": 0.0,
        },
        {
            "params": param_groups["other_decay"],
            "lr": default_lr,
            "weight_decay": training_args.weight_decay,
        },
        {
            "params": param_groups["other_nodecay"],
            "lr": default_lr,
            "weight_decay": 0.0,
        },
    ]
    ...
    optimizer = optim_class(optimizer_grouped_parameters, **optim_kwargs)
    
    logger.info_rank0(
        f"Using separate learning rates: selector_lr={selector_lr:.2e} ({selector_param_count:,} params), "
        f"default_lr={default_lr:.2e} ({other_param_count:,} params)."
    )
...
def create_custom_optimizer(
    ...
    if finetuning_args.selector_learning_rate is not None:
        return _create_selector_lr_optimizer(model, training_args, finetuning_args)
