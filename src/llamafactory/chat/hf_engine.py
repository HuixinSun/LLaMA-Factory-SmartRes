...
class HuggingfaceEngine(BaseEngine):
    ...
    def _process_args(
        ...
        # Expose latest user prompt for downstream processing
        # import pdb; pdb.set_trace()
        user_text = next((m.get("content") for m in reversed(messages) if m.get("role") == "user"), None)
        ...
        # Pass latest user text through to model.forward → visual.forward
        if isinstance(user_text, str) and len(user_text) > 0:
            gen_kwargs["text_prompt"] = user_text
        ...
        # Filter out any training-only keys that could break generate (e.g., labels)
    ...
    def _chat(
        ...
        # Set text_prompt directly on model before generation since generate() doesn't pass it through
        if "text_prompt" in gen_kwargs:
            text_prompt_value = gen_kwargs.pop("text_prompt")
            # Set on all levels of the model hierarchy
            setattr(model, "_last_text_prompt", text_prompt_value)
            if hasattr(model, 'model') and model.model != model:
                setattr(model.model, "_last_text_prompt", text_prompt_value)
            if hasattr(model, 'visual'):
                setattr(model.visual, "_last_text_prompt", text_prompt_value)
