...
class BaseModelArguments:
    ...
    use_multi_scale: bool = field(
        default=False,
        metadata={"help": "Whether to enable multi-scale processing for vision models."},
    )
    use_fastv: bool = field(
        default=False,
        metadata={"help": "Whether to enable FastV token pruning for Qwen2.5-VL (baseline comparison)."},
    )
    fastv_k: int = field(
        default=2,
        metadata={"help": "FastV: prune vision tokens after this decoder layer."},
    )
    fastv_r: float = field(
        default=0.5,
        metadata={"help": "FastV: fraction of vision tokens to KEEP after layer k (0.5 = FastV-50%)."},
    )
    fastv_inplace: bool = field(
        default=True,
        metadata={"help": "FastV: use the in-place (真剪) implementation rather than masking."},
    )
    teva_baseline: bool = field(
        default=False,
        metadata={"help": "Use TEVA baseline (bbox-based selection) instead of learned MTS router."},
    )
    scale_levels: int = field(
        default=2,
        metadata={"help": "Number of scale levels for multi-scale processing."},
    )
    conf_thresh: float = field(
        default=0.5,
        metadata={"help": "Confidence threshold for multi-scale region selection."},
    )
    scale_thresh: float = field(
        default=0.8,
        metadata={"help": "Scale threshold for multi-scale region selection."},
    )
    base_resolution: int = field(
        default=224,
        metadata={"help": "Base resolution for multi-scale processing."},
    )
    high_res_scale: float = field(
        default=2.0,
        metadata={"help": "High resolution scale factor for multi-scale processing (HR resolution = base_resolution * high_res_scale)."},
    )
    scale_layer: Optional[int] = field(
        default=None,
        metadata={"help": "Which transformer layer to use for scale prediction in multi-scale processing (default: uses model's default)."},
    )

    # SmartRes. These are the names used in the paper and in the smartres package; each
    # one maps onto the legacy field above so both spellings work.
    use_smartres: bool = field(
        default=False,
        metadata={"help": "Enable SmartRes dynamic resolution routing."},
    )
    tau: Optional[float] = field(
        default=None,
        metadata={"help": "SmartRes routing threshold, M = STE(S > tau)."},
    )
    router_layer: Optional[int] = field(
        default=None,
        metadata={"help": "Vision block whose features the SmartRes router reads."},
    )
    encode_snap: str = field(
        default="window",
        metadata={"help": "SmartRes encode-set granularity: 'window' or 'unit'."},
    )
    lr_budget: Optional[float] = field(
        default=None,
        metadata={"help": "SmartRes r_LR, the low-resolution token budget."},
    )
    hr_budget: Optional[float] = field(
        default=None,
        metadata={"help": "SmartRes r_HR, the high-resolution token budget."},
    )
    lambda_route: float = field(
        default=1.0,
        metadata={"help": "SmartRes weight of the routing BCE term."},
    )
    lambda_hinge: float = field(
        default=5.0,
        metadata={"help": "SmartRes weight of the margin regulariser."},
    )

    def _resolve_smartres(self):
        """Map the SmartRes names onto the fields the rest of the code reads."""
        if not self.use_smartres:
            return
        import os

        self.use_multi_scale = True
        os.environ.setdefault("MTS_TOKEN_ORDER", "smartres")
        os.environ["MTS_SPARSE_SNAP"] = self.encode_snap
        if self.tau is not None:
            self.conf_thresh = self.tau
            self.scale_thresh = self.tau
        if self.router_layer is not None:
            self.scale_layer = self.router_layer
        if self.lr_budget is not None and self.hr_budget:
            self.high_res_scale = self.lr_budget / self.hr_budget

    def __post_init__(self):
        self._resolve_smartres()
