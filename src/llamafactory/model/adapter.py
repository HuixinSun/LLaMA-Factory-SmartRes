...
def _setup_lora_tuning(
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        # Check if lora_target is valid (not empty, not "none_module", etc.)
        skip_lora = (
            not finetuning_args.lora_target or
            (len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] in ["", "none", "none_module", "false"])
        )
        
        if skip_lora:
            # No LoRA training, only train additional_target modules
            logger.info_rank0("Skipping LoRA setup (lora_target is empty or disabled)")
            logger.info_rank0(f"Only training additional_target modules: {finetuning_args.additional_target}")
            
            # Manually set additional_target modules as trainable
            if finetuning_args.additional_target:
                forbidden_modules = get_forbidden_modules(model.config, finetuning_args)
                for name, param in model.named_parameters():
                    # Check if this parameter belongs to any additional_target module
                    if any(target_name in name for target_name in finetuning_args.additional_target):
                        # Check if not in forbidden modules
                        if not any(forbidden_module in name for forbidden_module in forbidden_modules):
                        ...
                        # Not in additional_target, freeze it
                        param.requires_grad_(False)
            else:
                logger.warning_rank0("No additional_target specified, no parameters will be trained!")
        else:
            # Normal LoRA setup
