...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        # Multi-scale inputs from image processor
        ...
        # if hasattr(ip, "_last_processed_hr_resize_ratio"):
        #     ratios = ip._last_processed_hr_resize_ratio
        #     if isinstance(ratios, list) and ratios:
        #         valid = [r for r in ratios if r is not None and len(r) == 2]
        #         if valid:
        #             # Convert list to numpy array first to avoid slow conversion warning
        #             valid_array = np.array(valid, dtype=np.float32)
        #             mm_inputs["bbox_scale_xy"] = _torch.tensor(valid_array, dtype=_torch.float32)
        #             # remove
        #             ip._last_processed_hr_resize_ratio = None
        ...
        #### Build text prompts from labels (ignore IGNORE_INDEX) for training-time supervision or conditioning
        _labels = features.get("labels", None)
        if _labels is not None and torch.is_tensor(_labels) and hasattr(self, "tokenizer") and self.tokenizer is not None:
            prompts: list[str] = []
            ignore_val = IGNORE_INDEX
            mask = _labels.ne(ignore_val)
            for row, row_mask in zip(_labels, mask):
                ids = row[row_mask].tolist()
                text = self.tokenizer.decode(ids, skip_special_tokens=True) if ids else ""
                prompts.append(text)
            # Use singular key to match model.forward(text_prompt=...) signature
            features["text_prompt"] = prompts
        
        #### Extract the instruction (user query) from input_ids.
        # [2026-07-26] This used to decode the whole row, so during training
        # `instruction` also contained the assistant answer (the GT bboxes) while at
        # predict time it held the prompt only -- anything conditioning on it would
        # have leaked labels and been train/test inconsistent. Now: take the LAST user
        # turn only, which is identical in both phases and is where the referring
        # expression lives. Vision pad ids are dropped before decoding (a row carries
        # thousands of them and each renders to ~13 chars).
        _input_ids = features.get("input_ids", None)
        if _input_ids is not None and torch.is_tensor(_input_ids) and hasattr(self, "tokenizer") and self.tokenizer is not None:
            if not hasattr(self, "_vision_pad_ids"):
                _ids = self.tokenizer.convert_tokens_to_ids(
                    ["<|image_pad|>", "<|video_pad|>", "<|vision_start|>", "<|vision_end|>"]
                )
                self._vision_pad_ids = {i for i in (_ids if isinstance(_ids, list) else [_ids]) if isinstance(i, int) and i >= 0}

            instructions: list[str] = []
            for row in _input_ids:
                keep = row[~torch.isin(row, torch.tensor(sorted(self._vision_pad_ids), device=row.device))] \
                    if self._vision_pad_ids else row
                text = self.tokenizer.decode(keep.tolist(), skip_special_tokens=False)
                # last user turn; the template is <|im_start|>user\n ... <|im_end|>
                turns = re.findall(r"<\|im_start\|>user\n(.*?)<\|im_end\|>", text, flags=re.S)
                query = turns[-1] if turns else text
                instructions.append(re.sub(r"<\|[^|]*\|>", "", query).strip())
            features["instruction"] = instructions
