...
if is_pillow_available():
    ...
    # The EgoIntent/PACO frame dumps contain a few truncated JPEGs. Without this,
    # PIL raises "image file is truncated (N bytes not processed)" the moment
    # anything calls .load() on them (hit during training job 110337).
...
class MMPluginMixin:
    ...
    def _validate_input(
        ...
        # Only swap to MultiScaleImageProcessor if use_multi_scale is enabled
        # Check if processor has use_multi_scale attribute (set by patch_processor in patcher.py)
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            ...
            # If MultiScaleImageProcessor is active, attach base and HR frames deterministically
            import torch
            # import pdb;pdb.set_trace()
            if hasattr(image_processor, "_processed_base_patches"):
                base_patches = image_processor._processed_base_patches
                if base_patches is not None:
                    if not isinstance(base_patches, torch.Tensor):
                        base_patches = torch.as_tensor(base_patches)
                    mm_inputs["pixel_patches"] = base_patches
            if hasattr(image_processor, "_processed_hr_patches"):
                hr_patches = image_processor._processed_hr_patches
                if hr_patches is not None:
                    if not isinstance(hr_patches, torch.Tensor):
                        hr_patches = torch.as_tensor(hr_patches)
                    mm_inputs["pixel_patches_hr"] = hr_patches

            # Extract HR grid dimensions from MultiScaleImageProcessor
            if hasattr(image_processor, "_processed_hr_grid_thw"):
                hr_grid_thw = image_processor._processed_hr_grid_thw
                if hr_grid_thw is not None:
                    if not isinstance(hr_grid_thw, torch.Tensor):
                        hr_grid_thw = torch.as_tensor(hr_grid_thw, dtype=torch.int64)
                    mm_inputs["hr_grid_thw"] = hr_grid_thw
...
class Qwen2VLPlugin(BasePlugin):
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            ...
            bf = image_processor(images, return_tensors="pt")
