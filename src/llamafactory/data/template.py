...
register_template(
    ...
    format_tools=ToolFormatter(tool_format="qwen"),
    default_system=(
        "You are a helpful assistant for visual grounding tasks.\n\n"
        "HARD RULES — OUTPUT FORMAT\n"
        "1) Respond with a single fenced JSON code block ONLY (```json ... ```). No extra text.\n"
        "2) The JSON MUST be a non-empty array. It MUST contain ≥ 1 object.\n"
        "3) Each object MUST have exactly these keys:\n"
        "   - \"bbox_2d\": [x1, y1, x2, y2]  (numbers only; ASCII; x1<=x2, y1<=y2)\n"
        "   - \"label\": \"object\"  (use this exact ASCII string)\n"
        "   - \"confidence\": a float in [0,1]\n"
        "4) If uncertain, still output ONE best-guess box with low confidence (e.g., 0.25). Never return empty.\n"
        "5) ASCII only. No comments, no trailing commas, no text outside the JSON block.\n"
        "6) If any check fails, silently fix and re-emit a valid JSON block."
    ),
    stop_words=["<|im_end|>"],
    replace_eos=True,
    mm_plugin=get_mm_plugin(name="qwen2_vl", image_token="<|image_pad|>", video_token="<|video_pad|>"),
...
# custom template - strict bbox output format
register_template(
    name="qwen2_vl_bbox",
    format_user=StringFormatter(slots=["<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n"]),
    format_assistant=StringFormatter(slots=["{{content}}<|im_end|>\n"]),
